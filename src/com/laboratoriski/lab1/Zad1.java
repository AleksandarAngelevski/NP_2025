...
class Account{
    ...
    Account(String name, double balance) {
        ...
        // System.out.println(balance + "___");
    }
...
class FlatPercentProvisionTransaction extends Transaction{
    ...
    public int getPercent(){
        return this.centsPerDolar;
    }

    @Override
    public boolean equals(Object obj) {
        return this.getToId() == ((Transaction)obj).getToId() &&  this.getFromId() == ((Transaction)obj).getFromId();
    }
}
...
class Bank{
    ...
    public boolean makeTransaction(Transaction t){
        Account acc1 = Arrays.stream(accounts).filter((e)->e.getId() == t.getFromId()).toArray(Account[]::new)[0];
        Account acc2 = Arrays.stream(accounts).filter((e)->e.getId() == t.getToId()).toArray(Account[]::new)[0];
        // System.out.println("Making transaction");
        // System.out.println(acc1);
        // System.out.println(acc2);
        if(t.getAmount()<= acc1.getBalance() && (acc1 !=null && acc2!=null)){

            if(t instanceof FlatAmountProvisionTransaction){
                if(t.getAmount()+((FlatAmountProvisionTransaction) t).getFlatProvision()>acc1.getBalance())return false;
                totalProvision+=((FlatAmountProvisionTransaction)t).getFlatProvision();
                acc1.setBalance(acc1.getBalance() - (t.getAmount()+((FlatAmountProvisionTransaction) t).getFlatProvision()));
            }else{
                if(t.getAmount()+(t.getAmount()*(((FlatPercentProvisionTransaction) t).getPercent()/100.0))>acc1.getBalance())return false;
                totalProvision+=(t.getAmount() * ((FlatPercentProvisionTransaction)t).getPercent())/100.0;
                acc1.setBalance(acc1.getBalance() - (t.getAmount()+((t.getAmount()*((FlatPercentProvisionTransaction) t).getPercent()/100.0))));
            }
            acc2.setBalance(acc2.getBalance() + t.getAmount());
            totalTransfer+=t.getAmount();
    ...
    @Override
    public String toString() {
        String acconts = Arrays.stream(accounts)
    ...
    public boolean equals(Object o) {
        if (o == null || getClass() != o.getClass()) return false;
        Bank bank = (Bank) o;
        return Double.compare(totalTransfer, bank.totalTransfer) == 0 && Double.compare(totalProvision, bank.totalProvision) == 0 && Objects.equals(name, bank.name) && Objects.deepEquals(accounts, bank.accounts);
    }

    @Override
    public int hashCode() {
        return Objects.hash(name, Arrays.hashCode(accounts), totalTransfer, totalProvision);
    }
