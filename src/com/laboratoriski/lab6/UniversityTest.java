...
//        System.out.println(this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().toList());
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).map(Course::getName).distinct().toList();
    }

    public List<Course> getCoursesWithMinCredits(int minCredits) {

        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().filter(e->e.getCredits()>=minCredits).collect(Collectors.toList());
    }

    public int getTotalStudentCount() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).mapToInt(Course::getEnrolledStudents).sum();
    }

    public Optional<Course> getHardestCourse() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().max(Comparator.comparing(Course::getDifficulty));
        //TODO
    }

    public Map<Integer, List<Course>> groupByDifficulty() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().collect(Collectors.groupingBy(Course::getDifficulty));
        //TODO
    }

    public Map<String, Integer> getCourseEnrollmentMap() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().collect(Collectors.toMap(Course::getCode,Course::getEnrolledStudents));
        //TODO
    }

    public double getAverageEnrollmentPerCourse() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct()
                .mapToInt(Course::getEnrolledStudents).sum()*1.00/this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().collect(Collectors.toList()).size();
        //TODO
    }

    public List<String> getSortedCourseCodes() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().map(Course::getCode).sorted().collect(Collectors.toList());
        //TODO
    }

    public Map<String, List<String>> getDepartmentToCourseNames() {
        return this.departments.stream().collect(
    ...
    public List<Course> getAllCourses() {
        return this.departments.stream().map(Department::getCourses).flatMap(List::stream).distinct().toList();
    }

    public Optional<Department> getMostPopularDepartment() {
        return this.departments.stream().max(
                Comparator.comparing(
                        dept ->
                            dept.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()
                ));
    }

    public Map<Integer, Integer> getStudentsByDifficulty() {
//        System.out.println( this.departments.stream().map(Department::getCourses).flatMap(List::stream)
//                .collect(Collectors.groupingBy(
//                        Course::getDifficulty,
//                        Collectors.summingInt(Course::getEnrolledStudents)
//                        )));
        return this.departments.stream().flatMap(l -> l.getCourses().stream())
                .collect(Collectors.groupingBy(
                        Course::getDifficulty,
                        Collectors.summingInt(Course::getEnrolledStudents)
                ));
...
//
    public University mergeFourSmallestDepartments() {
        List<Department> last4 = this.departments.stream().sorted(
                Comparator.comparing((d)->
                        d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum())
        ).limit(4).collect(Collectors.toList());
        String newName = last4.stream().map(Department::getName).collect(Collectors.joining(" & "));

        Department merged = new Department(
                last4.stream().map(Department::getName).collect(Collectors.joining(" & ")),
                last4.stream().map(Department::getCourses).flatMap(List::stream).distinct().collect(Collectors.toList()));

        List<Department> departmentsWithoutLast4 = Stream.concat(
                this.departments.stream().sorted(Comparator.comparing(
                        (d) -> d.getCourses().stream().distinct().map(Course::getEnrolledStudents).mapToInt(Integer::valueOf).sum()
                )).skip(4),
                Stream.of(merged)
...
//        System.out.println("Smallest four departments");
//        System.out.println(this.departments.stream().sorted(Comparator.comparing((d)->
//            d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum())
//        ).limit(4).map(Department::getCourses)
//                        .flatMap(List::stream).distinct()
//                .collect(Collectors.toList()));
        return new University(departmentsWithoutLast4);
        //TODO
    }
}
