...
class LogSystem<T extends ILog> {
    ...
    LogSystem(ArrayList<T> elements) {
...
void printResults() {
    LogProcessor<T> firstLogProcessor = new  LogProcessor<T>() {
        ...
        public ArrayList<T> processLogs(ArrayList<T> logs) {
            return logs.stream().filter(e -> e.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
        }
    };

            LogProcessor<T> secondLogProcessor = new  LogProcessor<T>() {
                @Override
                public ArrayList<T> processLogs(ArrayList<T> logs) {
                    return logs.stream().filter(e -> e.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));
                }
            };

                    LogProcessor<T> thirdLogProcessor = new   LogProcessor<T>() {
                        @Override
                        public ArrayList<T> processLogs(ArrayList<T> logs) {
                            return logs.stream().sorted(Comparator.comparingLong(ILog::getTimestamp)).collect(Collectors.toCollection(ArrayList::new));
                        }
                    };
